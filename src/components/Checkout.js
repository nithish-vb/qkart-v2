...
/**
 * @typedef {Object} Product - Data on product available to
 *
    ...
    <>
      ...
      <Grid container>
        <Grid item xs={12} md={9}>
          <Box className="shipping-container" minHeight="100vh">
            ...
            {addresses.all.length === 0 ? (
              ...
              addresses.all.map((ad) => (
                <Box
                  display="flex"
                  justifyContent="space-between"
                  className={`address-item ${
                    addresses.selected === ad._id ? "selected" : "not-selected"
                  }`}
                  onClick={() =>
                    setAddresses({ ...addresses, selected: ad._id })
                  }
                >
                  <Typography>{ad.address}</Typography>
                  <Button
                    className="explore-button"
                    type="text"
                    onClick={() => deleteAddress(token, ad._id)}
                  >
                    <Delete />
                    DELETE
                  </Button>
        ...
        <Grid item xs={12} md={3} bgcolor="#E9F5E1">
          <Cart readonly={true} products={products} items={items} />
        </Grid>
      </Grid>
      <Footer />
    </>
